...
这里额外说一个处理二维数组的常用小技巧，你有时会看到使用「方向数组」来处理上下左右的遍历，和前文 [图遍历框架](https://labuladong.gitee.io/plugin-v3/?qno=797) 的代码很类似：
...
![](https://pic.leetcode-cn.com/1644371573-FXmenf-1.jpg)
...
> PS：这类 DFS 算法还有个别名叫做 [FloodFill 算法](https://labuladong.gitee.io/plugin-v3/?qno=733)，现在有没有觉得 FloodFill 这个名字还挺贴切的~
...
![](https://pic.leetcode-cn.com/1644371573-ztKKQX-2.png)
...
> PS：处理这类岛屿题目除了 DFS/BFS 算法之外，Union Find 并查集算法也是一种可选的方法，前文 [Union Find 算法运用](https://labuladong.gitee.io/plugin-v3/?qno=130) 就用 Union Find 算法解决了一道类似的问题。
...
![](https://pic.leetcode-cn.com/1644371574-mKvZWQ-3.jpg)
...
![](https://pic.leetcode-cn.com/1644371573-XVKBnl-4.jpg)

**这道题的关键在于，如何快速判断子岛屿**？肯定可以借助 [Union Find 并查集算法](https://labuladong.gitee.io/plugin-v3/?qno=323) 来判断，不过本文重点在 DFS 算法，就不展开并查集算法了。
...
![](https://pic.leetcode-cn.com/1644371573-DUyTSt-5.jpg)

其中有四个岛屿，但是左下角和右上角的岛屿形状相同，所以不同的岛屿共有三个，算法返回 3。

很显然我们得想办法把二维矩阵中的「岛屿」进行转化，变成比如字符串这样的类型，然后利用 HashSet 这样的数据结构去重，最终得到不同的岛屿的个数。

如果想把岛屿转化成字符串，说白了就是序列化，序列化说白了就是遍历嘛，前文 [二叉树的序列化和反序列化](https://labuladong.gitee.io/plugin-v3/?qno=297) 讲了二叉树和字符串互转，这里也是类似的。
...
![](https://pic.leetcode-cn.com/1644371573-xTHusD-6.png)
...
`dir` 记录方向，`dfs` 函数递归结束后，`sb` 记录着整个遍历顺序，其实这就是前文 [回溯算法核心套路](https://labuladong.gitee.io/plugin-v3/?qno=51) 说到的回溯算法框架，你看到头来这些算法都是相通的。
...
这样，这道题就解决了，至于为什么初始调用 `dfs` 函数时的 `dir` 参数可以随意写，这里涉及 DFS 和回溯算法的一个细微差别，前文 [图算法基础](https://labuladong.gitee.io/plugin-v3/?qno=797) 有写，这里就不展开了。
...
> **[点击我的头像](https://labuladong.gitee.io/algo/) 查看更多优质算法文章，手把手带你刷力扣，致力于把算法讲清楚！我的 [算法教程](https://github.com/labuladong/fucking-algorithm) 已经获得 100k star，欢迎点赞！**

# 2.Java dfs 超过97%
![图片.png](https://pic.leetcode-cn.com/1651133792-jodwcc-%E5%9B%BE%E7%89%87.png)
...
判断岛屿是否相同，可以用HashSet记录下每个岛屿遍历时的路径即可，最后统计HashSet的长度即可。

通过深度优先遍历，由于每个岛屿中的陆地的遍历顺序是由代码中的顺序决定的，所以，每一个岛屿都有一个与之对应的遍历顺序，所以，记录下进入和离开该节点方向的序号即可，将进入和离开的序号作为岛屿中的遍历路径。
...
class Solution:
    ...
    def dfs(self, grid, i, j, path):
        ...
        return path
```
# 4.(Java)深度优先搜索+HashSet去重
**执行用时：6 ms, 在所有 Java 提交中击败了97.66%的用户**
**内存消耗：40.1 MB, 在所有 Java 提交中击败了94.74%的用户**
## 本题思路：
